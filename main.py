...
train, valid, test = dataset.split(split_ratio=[0.7, 0.1, 0.2], random_state=random.getstate())
...
train_it, valid_it, test_it = BucketIterator.splits((train, valid, test), batch_size=bsize, sort_key=lambda x: len(x.jp), sort_within_batch=False, device=device)
